...
void MainWidget::prepare(struct event_data ed[],int len,QSize siz,QString netf,QList<int> favss){
    ...
    for(int i=0;i<len;i++){
        if(ed[i].etime==prev_time && type==0){
            ...
            par_idx++;
            items=par_idx;
            if(dodebug)qDebug()<<"make widget:"<<par_idx;
        }
    }
...
void MainWidget::paintGL(){
    ...
    } else if(setting_face)qDebug()<<"FACER BLOCKED";
    if(items==0)return;
...
void MainWidget::mousePressEvent(QMouseEvent *e){
    if(!timer->isActive()){
        if(dodebug)qDebug()<<"TIMEREV:Start timer";
        timer->start(17);
    }
    if(items==0 || mw<0)return;
    if(mw==-1){
        if(dodebug)qDebug()<<"MW=-1 return";
        return;
    }
    p_stop_emitted=false;
    unset_slave();
    ...
    scroll_locked=false;
    if(!scrolling){
        if(type==0 && player_inited){
            qDebug()<<"VOL:"<<volume2_rect.contains(e->pos())<<volume2_rect<<e->pos();
            if(is_playing()){
        ...
        if(button2_rect.contains(e->pos())){
            if(dodebug)qDebug()<<"ADD FAV CLICK";
            emit add_fav_click();
...
//        }
//    }
//    if(scr==0 && !pressed && rotate_to_element==-1 && all_loaded){
//        qDebug()<<"TIMER STOP:ALL SET";
//        timer->stop();
//    }
//    scr=0;
//    hscroll_autopilot=0;
...
void MainWidget::timerev(){
    ...
    // qDebug()<<"QQ"<<scr<<!pressed<<!need_seek<<!need_play<<all_loaded<<(rotate_to_element==-1);
    if(scr==0 && !pressed && rotate_to_element==-1 && all_loaded && !is_playing() & timer->isActive()){
        if(dodebug)qDebug()<<"TIMER STOP:TIMEREV";
        timer->stop();
    }
    scr=0;
    hscroll_autopilot=0;
...
void MainWidget::get_coords(){
    ...
    if(!setting_face && !all_loaded){
        ...
        if(widget[mw].loaded<2)
            ...
            genw=processing;
        if(genw>-1){
            if(QFileInfo::exists(filename[genw])){
                widget[genw].loaded=2;
                widget[genw].image=new QImage(QImage(filename[genw]).scaled(msize,aspect,Qt::SmoothTransformation));
                widget[genw].image->setDevicePixelRatio(dpi);
                if(pdodebug)qDebug()<<"Loaded image:"<<widget[genw].image->size()<<widget[genw].image->sizeInBytes()<<filename[genw]<<","<<genw;
                update();
            }  else {
                setting_face=false;
            }
...
void MainWidget::set_player(){
    if(player==nullptr){
        ...
        video=new MyVideoSurface;
        video->setState(true);
        player->setVideoOutput(video);
        catch_stop=true;
    }
    need_stop_update=false;
    qDebug()<<"URL:"<<player_url;
    ...
    qDebug()<<"PLAYERR:"<<player->isMuted()<<player->volume();
    player->pause();
    if(player_enabled){
...
void MainWidget::seek(qint64 curr_pos,bool){
    ...
    if(player_enabled)
        if(player->position()==start_time){
            if(pdodebug)qDebug()<<"Seek not needed!";
        } else {
            player->setPosition(start_time);
            if(pdodebug)qDebug()<<"Media PLAYER:TRIGGER SEEK TO "<<start_time<<player->position()<<player->state()<<player_offset;
        }
}
void MainWidget::stop(){
    if(!player_inited || !player_enabled)return;
    if(pdodebug)qDebug()<<"Media PLAYER:TRIGGER STOP AT "<<player->position()<<player->state();
    if(player->state()==QMediaPlayer::PausedState){
        if(pdodebug)qDebug()<<"Already stopped";
    } else {
        player->pause();
    }
    ...
    show_buffer=false;
    seek(end_time);
    if(need_stop_update)update();
}
void MainWidget::setImage(const QImage& im,bool swrgb){
    frames_cnt++;
    if(first_frame){
        if(pdodebug)
            qDebug()<<"Media PLAYER:Transport START - IMG PARAMS:"<<im.sizeInBytes()<<","<<im.size()<<","<<im.depth();
        first_frame=false;
    }
    if(swrgb)
        vim=im.scaled(msize,Qt::IgnoreAspectRatio).rgbSwapped();
    else
        vim=im.scaled(msize,Qt::IgnoreAspectRatio);
    player_inited=true;
    if(player->position()>end_time || need_stop || catch_stop){
        if(catch_stop){
            if(pdodebug)qDebug()<<"CATCH STOP LAST FRAME";
        }
        if(pdodebug)qDebug()<<"Media PLAYER:End of media catch avg "<<(frames_cnt*1000/player_offset)<<"fps ["<<frames_cnt<<" frames at "<<player_offset<<"ms]";
        stop();
        return;
    }
    is_video_frame=true;
...
void MainWidget::media_info(QMediaPlayer::MediaStatus s){
    if(pdodebug)qDebug()<<"Media PLAYER STATUS:"<<s;
    if(QMediaPlayer::InvalidMedia == s){
        player_enabled=false;
        is_video_frame=false;
        update();
    }
    if(QMediaPlayer::BufferingMedia == s){
        player_inited=true;
        update();
    }
}
...
void MainWidget::state_changed(QMediaPlayer::State state){
    playing=false;
    if(pdodebug)qDebug()<<"Media PLAYER STATE:"<<state;
    if(state==QMediaPlayer::PausedState && is_video_frame){
        if(pdodebug)qDebug()<<"Media PLAYER:Flush overtime frame";
        is_video_frame=false;
        update();
    } else
        if(state==QMediaPlayer::PlayingState ){
            if(pdodebug)qDebug()<<"Media PLAYER:Re-init start flag and counter";
            first_frame=true;
            playing=true;
            frames_cnt=0;
        }
...
void MainWidget::media_buffer_filled_info(int perc){
    if(prev_buffer!=perc){
        if(pdodebug)qDebug()<<"Media PLAYER BUFFER"<<perc<<player->position()<<player->state();
        prev_buffer=perc;
